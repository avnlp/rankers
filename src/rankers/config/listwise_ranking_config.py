"""Pydantic configuration models for the listwise ranking pipeline."""
...
class ListwiseOpenAIConfig(BaseModel):
    ...
    api_keys: list[str] | None = Field(
        default=None,
        description="List of OpenAI API keys to use for LLMs. Required for OpenAI-based rankers.",
    )
    key_start_id: int | None = Field(
        default=None,
        description="Start index for OpenAI API keys. Useful for key rotation.",
    )
    proxy: str | None = Field(
        default=None, description="Proxy for OpenAI API requests."
    )
    api_type: str | None = Field(
        default=None, description="API type for OpenAI API requests."
    )
    api_base: str | None = Field(
        default=None, description="API base URL for OpenAI API requests."
    )
    api_version: str | None = Field(
        default=None, description="API version for OpenAI API requests."
    )
...
class ListwiseHFConfig(BaseModel):
    ...
    model_path: str = Field(
        ..., description="Hugging Face model name/path for the LLM ranker."
    )
    ...
    context_size: int = Field(
        default=4096,
        gt=0,
        description="Maximum context size in tokens that the model can handle.",
    )
    ...
    top_k: int = Field(
        default=10,
        gt=0,
        description="Maximum number of documents to rerank and return.",
    )
    ...
    variable_passages: bool = Field(
        default=False,
        description="Whether to allow variable number of passages per request.",
    )
    sliding_window_size: int = Field(
        default=20,
        gt=0,
        description="Size of sliding window for processing long documents.",
    )
    sliding_window_step: int = Field(
        default=10, gt=0, description="Step size for sliding window movement."
    )
    ...
    openai: ListwiseOpenAIConfig | None = Field(
        default=None,
        description="OpenAI API configuration. Required for OpenAI-based rankers.",
    )
