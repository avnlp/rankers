...
class SetwiseMethod(str, Enum):
    """Available setwise ranking methods."""

    HEAPSORT = "heapsort"
    BUBBLESORT = "bubblesort"


class SetwiseLLMConfig(BaseModel):
    ...
    method: SetwiseMethod = Field(
        ...
        description="Sorting method to be used. Heapsort: efficient tree-based sorting, "
    ...
    num_permutation: int = Field(
        default=1,
        ge=1,
        description="Number of permutations for reranking.",
    )
    num_child: int = Field(
        default=3,
        ge=2,
...
class SetwiseRankingConfig(BaseModel):
    """Configuration for the setwise ranking pipeline.

    This configuration includes all settings needed to run the setwise ranking pipeline,
    ...
    # LLM configuration
    llm: SetwiseLLMConfig = Field(..., description="Setwise LLM ranker configuration")
